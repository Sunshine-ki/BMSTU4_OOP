...
// Поступило нажатие кнопки, обрабатываем его, добавляя в очередь и помечая этаж нужным
// образом
...
// данный слот помечает этаж false и, если есть еще этажи, которые true (то есть
// которые требуется посетить), то этот слот также генерирует сигнал о новой цели
// для кабины
